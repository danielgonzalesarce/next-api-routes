...
export async function PUT(
    ...
    try {
        ...
        if (error.code === 'P2025') return NextResponse.json({ error: 'Libro no encontrado' }, { status: 404 })
        if (error.code === 'P2002') return NextResponse.json({ error: 'El ISBN ya existe' }, { status: 409 })
